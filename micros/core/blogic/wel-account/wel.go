...
func AddWelAccount(address, status string) error {
	log.Info().Msgf("[welAccount logic internal] Creating welAccount %s...", address)

	if !verifyAddress(address) {
		err := model.ErrWelInvalidAddress
		log.Err(err).Msgf("[welAccount logic internal] Address %s invalid", address)
		return err
	}

	err := welDAO.AddWelAccount(address, status)
	if err != nil {
		log.Err(err).Msgf("[welAccount logic internal] Failed to create welAccount %s", address)
		return err
	}
	return nil
}
...
func RemoveWelAccount(address string) error {
	log.Info().Msgf("[welAccount logic internal] Start removing welAccount %s...", address)
	_, err := welDAO.GetWelAccount(address)
	if err != nil {
		log.Err(err).Msgf("[welAccount logic internal] Failed to retrieve welAccount %s", address)
		return err
	}

	log.Info().Msgf("[welAccount logic internal] Removing welAccount %s...", address)
	if err := welDAO.RemoveWelAccount(address); err != nil {
		log.Err(err).Msgf("[welAccount logic internal] Failed to remove welAccount %s", address)
		return err
	}
...
func GetAllWelAccounts(offset uint, size uint) ([]model.WelAccount, error) {
	log.Info().Msgf("[welAccount logic internal] Getting welAccounts...")
	welAccounts, err := welDAO.GetAllWelAccounts(offset, size)
	if err != nil {
		log.Err(err).Msg("[welAccount logic internal] Failed to retrieve welAccounts")
		return nil, err
	}
...
func GetAllRoles() ([]string, error) {
	log.Info().Msgf("[welAccount logic internal] Getting all roles...")
	roles, err := welDAO.GetAllRoles()
	if err != nil {
		log.Err(err).Msg("[welAccount logic internal] Failed to retrieve roles")
		return nil, err
	}
...
func GetWelAccountsWithRole(role string, offset uint, size uint) ([]model.WelAccount, error) {
	log.Info().Msgf("[welAccount logic internal] Getting welAccounts with role %s...", role)
	welAccounts, err := welDAO.GetWelAccountsWithRole(role, offset, size)
	if err != nil {
		log.Err(err).Msg("[welAccount logic internal] Failed to retrieve welAccounts with role " + role)
		return nil, err
	}
...
func GrantRole(address, role string, callerkey string) (string, error) {
	log.Info().Msgf("[welAccount logic internal] Start granting role %s to welAccount %s...", role, address)

	callerAddress, err := libs.KeyToB58Addr(callerkey)

	if err != nil {
		log.Err(err).Msg("[welAccount logic internal] Invalid private key")
		return "", err // invalid key
	}

	log.Info().Msgf("[welAccount logic internal] caller address: %s", callerAddress)

	if strings.HasPrefix(address, "0x") {
		address, err = libs.HexToB58(address)
		if err != nil {
			log.Err(err).Msg("[welAccount logic internal] Invalid address")
			return "", err
		}
	}
	...
	if err != nil {
		log.Err(err).Msgf("[welAccount logic internal] Unable to get wel account %s from DB", callerAddress)
		return "", err // invalid key
	}
	if acc.Status != "ok" {
	...
	// call workflow
	log.Info().Msgf("[welAccount logic internal] Calling GovContractService workflow...")
	...
	if err != nil {
		log.Err(err).Msg("[welAccount logic internal] Unable to call GrantRoleWorkflow")
		return "", err
	}
	...
	if err := we.Get(ctx, &txhash); err != nil {
		log.Err(err).Msg("[welAccount logic internal] GrantRoleWorkflow failed")
		return txhash, err
	}
	return txhash, nil
}
...
func RevokeRole(address, role string, callerkey string) (string, error) {
	log.Info().Msgf("[welAccount logic internal] Start revoking role %s to welAccount %s...", role, address)
	...
	if err != nil {
		log.Err(err).Msg("[welAccount logic internal] Unable to call RevokeRoleWorkflow")
		return "", err
	}
	log.Info().Str("Workflow", we.GetID()).Str("runID=", we.GetRunID()).Msg("dispatched")
	...
	var txhash string
	if err := we.Get(ctx, &txhash); err != nil {
		log.Err(err).Msg("[welAccount logic internal] RevokeRoleWorkflow failed")
		return txhash, err
	}
	return txhash, nil
}
...
func SetWelAccountStatus(address, status string) error {
	log.Info().Msgf("[welAccount logic internal] Start setting status %s to welAccount %s...", status, address)

	log.Info().Msgf("[welAccount logic internal] setting status %s to welAccount %s...", status, address)
	if err := welDAO.SetWelAccountStatus(address, status); err != nil {
		log.Err(err).Msgf("[welAccount logic internal] Failed to set status %s to welAccount %s", status, address)
		return err
	}
...
func SetCurrentAuthenticator(prikey string) error {
	...
	log.Info().Msgf("[welAccount logic internal] Set current authenticator to %s", address)
	...
	if err != nil {
		log.Err(err).Msgf("[welAccount logic internal] couldn't retrieve authenticator accounts")
		return err
	}
	match := libs.DropWhile(func(a model.WelAccount) bool { return a.Address != address }, accs)
	if len(match) < 1 {
		err = model.ErrWelAccountNotFound
		log.Err(err).Msgf("[welAccount logic internal] authenticator %s not found", address)
		return err
	}

	if match[0].Status != "ok" {
		err = model.ErrWelAccountLocked
		log.Err(err).Msgf("[welAccount logic internal] authenticator %s is locked", address)
		return err
	}
